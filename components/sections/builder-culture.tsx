...
export function BuilderCulture() {
  return (
    <section id="builder-culture" className="bg-[#060708] py-20 lg:py-[112px]">
      <Container>
        <FadeInView>
          ...
          <h2 className="font-heading mt-6 text-[44px] font-extrabold leading-[120%] tracking-[-1.2%] text-[#F0F2ED] sm:text-[40px] max-w-[650px]">
            We build products and the<br className="hidden lg:block" /> people who build them.
          </h2>
          <p className="mt-6 lg:text-[17px] text-sm leading-[150%] text-[#8F999E] sm:text-base max-w-[650px]">
            Interns, NYSC placements, and junior engineers learn through<br className="hidden lg:block" /> real work, real products, and real stakes.
          </p>
        </FadeInView>
        ...
        <div className="mt-16 grid gap-[18px] lg:grid-cols-3">
          {builderCultureItems.map((item, i) => (
            <FadeInView key={item.title} delay={i * 0.1}>
              <article
                className={cn(
                  "h-full rounded-[18px] shadow-sm bg-[#0E0F11] border border-[#333B4033] p-8 lg:p-[32px] transition-all duration-300",
                  "hover:border-accent/25 hover:bg-surface-elevated"
                )}
              >
                <h3 className="font-heading text-[20px] font-bold text-[#F0F2ED]">
                  {item.title}
                </h3>
                <p className="mt-4 lg:text-sm text-xs leading-[150%] text-[#8F999E]">
                  {item.description}
                </p>
              </article>
            </FadeInView>
